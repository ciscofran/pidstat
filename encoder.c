...
#include </usr/lib/gcc/x86_64-linux-gnu/4.9/include/stdatomic.h>
#include <sys/time.h>
...
#ifdef ENABLE_PTHREADS
pthread_mutex_t fllock = PTHREAD_MUTEX_INITIALIZER;
#endif


//#define ENABLE_PRINT_DEBUG 1
//#define ENABLE_PRINT_DEBUG_C 1
...
//  
struct timespec tsf_enq,tsf_deq,tsf_p;
struct timespec tsf_ddeq,tsf_dp;
struct timespec tsf_cdeq,tsf_cp;
struct timespec tsf_rdeq,tsf_rp;
clockid_t cidf=CLOCK_MONOTONIC;
//cidf = CLOCK_REALTIME;
...
#define MAXSTORQ 700000
#define MAXMONQ  2000
//
  atomic_ulong f_enqueTime[MAXSTORQ];
  atomic_ulong fr_dequeTime[MAXSTORQ];
  atomic_ulong fr_enqueTime[MAXSTORQ];
  atomic_ulong dd_dequeTime[MAXSTORQ];
  atomic_ulong dd_enqueTime[MAXSTORQ];
  atomic_ulong c_dequeTime[MAXSTORQ];
  atomic_ulong c_enqueTime[MAXSTORQ];
  atomic_ulong r_dequeTime[MAXSTORQ];
  atomic_ulong r_writeTime[MAXSTORQ];
  atomic_ulong fr_qWaitTime[MAXSTORQ];
  atomic_ulong dd_qWaitTime[MAXSTORQ];
  atomic_ulong c_qWaitTime[MAXSTORQ];
  atomic_ulong r_qWaitTime[MAXSTORQ];
  atomic_ulong fr_qProcessTime[MAXSTORQ];
  atomic_ulong dd_qProcessTime[MAXSTORQ];
  atomic_ulong c_qProcessTime[MAXSTORQ];
  atomic_ulong r_qProcessTime[MAXSTORQ];
  atomic_ulong f_ctr=0;
  atomic_ulong fr_ctr=0;
  atomic_ulong dd_ctr=0;
  atomic_ulong c_ctr=0;
  atomic_ulong r_ctr=0;
  //   
  atomic_ulong fr_qWaitTimeInterval[MAXMONQ];
  atomic_ulong dd_qWaitTimeInterval[MAXMONQ];
  atomic_ulong c_qWaitTimeInterval[MAXMONQ];
  atomic_ulong r_qWaitTimeInterval[MAXMONQ];
  atomic_ulong fr_qProcessTimeInterval[MAXMONQ];
  atomic_ulong dd_qProcessTimeInterval[MAXMONQ];
  atomic_ulong c_qProcessTimeInterval[MAXMONQ];
  atomic_ulong r_qProcessTimeInterval[MAXMONQ];
  atomic_int   monitor_ctr=0;
  atomic_ulong dd_qsize[MAXMONQ];
  atomic_ulong fr_qsize[MAXMONQ];
  atomic_ulong c_qsize[MAXMONQ];
  atomic_ulong r_qsize[MAXMONQ];

#define MAX_DD_QUEUE_SAMPLING 10
...
static void print_stats(stats_t *s) {
  ...
  for(unit_idx=0; unit_idx<unit_str_size; unit_idx++) {
    unsigned long unit_div_next = unit_div * 1024;
    //printf("%lu\n",unit_div_next);
  ...
  printf("Total number of chunks:        %16u\n", (nTotalChunks));
...
void pclock(char *msg, clockid_t cid)
{
  ...
  if (clock_gettime(cid,&ts) == -1)
     EXIT_TRACE("clock gettime failure\n");
...
void *Compress(void * targs) {
  ...
  atomic_uint c_dq_ctr = 0;
  atomic_uint c_enq_ctr = 0; 
  u_long startT;
  u_long endT; 
  atomic_uint elapsedT;  
  int i;

  struct timespec tsc_s,tsc_e,start,end;
  clockid_t cidc;
  cidc = CLOCK_MONOTONIC;
  struct timeval te;

  atomic_uint ctid = syscall(SYS_gettid); 
  ...
  while(1) {
    ...
    if(chunk->isLastL2Chunk){
       ...
       pthread_mutex_lock(&fllock);
       clock_gettime(cidf,&tsf_cdeq);
       //if (chunk->sequence.l1num == 5){
       //  printf("%d) Compress chunk 5 dequeue time:%lu,%lu,total=%.2f ns\n",ctid,tsf_cdeq.tv_sec,tsf_cdeq.tv_nsec,((tsf_cdeq.tv_sec*1.0e9)+tsf_cdeq.tv_nsec));
       //  printf("%d) Compress chunk 5 Wait Time= %.2f nanoseconds\n",ctid,((tsf_cdeq.tv_sec*1.0e9)+tsf_cdeq.tv_nsec)- dd_enqueTime[chunk->sequence.l1num]);
       //}       
       c_dequeTime[chunk->sequence.l1num] = ((tsf_cdeq.tv_sec*1.0e9)+tsf_cdeq.tv_nsec);
       c_qWaitTime[chunk->sequence.l1num] = (c_dequeTime[chunk->sequence.l1num] - dd_enqueTime[chunk->sequence.l1num]);
       pthread_mutex_unlock(&fllock);
    ...
    if (ringbuffer_isFull(&send_buf)) {
      ...
      if(chunk->isLastL2Chunk){
        clock_gettime(cidf,&tsf_cp);
        //if (chunk->sequence.l1num == 5){
        // printf("%d) Compress chunk 5 enqueue time%lu,%lu,total %.2f ns\n",ctid,tsf_cp.tv_sec,tsf_cp.tv_nsec,((tsf_cp.tv_sec*1.0e9)+tsf_cp.tv_nsec));
        // printf("%d) Compress chunk 5 Process Time= %.2f nanoseconds\n",ctid,((tsf_cp.tv_sec*1.0e9)+tsf_cp.tv_nsec)- c_dequeTime[chunk->sequence.l1num]);
        // }
        c_enqueTime[chunk->sequence.l1num] = ((tsf_cp.tv_sec*1.0e9)+tsf_cp.tv_nsec);
        c_qProcessTime[chunk->sequence.l1num] = (c_enqueTime[chunk->sequence.l1num]-c_dequeTime[chunk->sequence.l1num]);
        c_ctr++;
      }
  }
...
void * Deduplicate(void * targs) {
  ...
  chunk_t *chunk;
  int r;
  atomic_uint dd_dq_ctr = 0;
  atomic_uint dd_enq_ctr = 0; 
  ...
  struct timespec tsf_s,tsf_e;
  struct timespec tsd_s,tsd_e;
  struct timespec Start,End;
  atomic_ulong St_sec,End_sec;
  atomic_ulong St_ns,End_ns;
  atomic_ulong threadtime;

  clockid_t cidd;
  cidd = CLOCK_MONOTONIC;
  time_t clk = time(NULL);
  struct timeval te;

  atomic_uint dtid = syscall(SYS_gettid); 

  clock_gettime(cidd,&Start);//measure thread elapsed time
  St_sec = Start.tv_sec;
  St_ns  = Start.tv_nsec;
  
  ringbuffer_t recv_buf, send_buf_reorder, send_buf_compress;

  //ProfilerStart("flenque.log");
#ifdef ENABLE_STATISTICS
  stats_t *thread_stats = malloc(sizeof(stats_t));
  ...
  while (1) {
    ...
    if(chunk->isLastL2Chunk){
       clock_gettime(cidf,&tsf_ddeq);
       //if (chunk->sequence.l1num == 5) {
       //  printf("%d) Dedup chunk 5 dequeue time:%lu,%lu,total=%.2f ns\n",dtid,tsf_ddeq.tv_sec,tsf_ddeq.tv_nsec,((tsf_ddeq.tv_sec*1.0e9)+tsf_ddeq.tv_nsec));
       //  printf("%d) Dedup chunk 5 Wait Time= %.2f nanoseconds\n",dtid,((tsf_ddeq.tv_sec*1.0e9)+tsf_ddeq.tv_nsec)- fr_enqueTime[chunk->sequence.l1num]);

      //   printf("%d) Dedup last chunk 5 DD deQUEUE   : ",dtid );
      //   ringbuffer_checksize(&deduplicate_que[qid].buf);

      // }
       dd_dequeTime[chunk->sequence.l1num] = ((tsf_ddeq.tv_sec*1.0e9)+tsf_ddeq.tv_nsec);
       dd_qWaitTime[chunk->sequence.l1num] = (dd_dequeTime[chunk->sequence.l1num] - fr_enqueTime[chunk->sequence.l1num]);  
    }
    ...
    if(chunk->isLastL2Chunk){
       clock_gettime(cidf,&tsf_dp);
       //if (chunk->sequence.l1num == 5) {
       //  printf("%d) Dedup chunk 5 enqueue time:%lu,%lu,total=%.2f ns\n",dtid,tsf_dp.tv_sec,tsf_dp.tv_nsec,((tsf_dp.tv_sec*1.0e9)+tsf_dp.tv_nsec));
       //  printf("%d) Dedup chunk 5 Process Time= %.2f nanoseconds\n",dtid,((tsf_dp.tv_sec*1.0e9)+tsf_dp.tv_nsec)- dd_dequeTime[chunk->sequence.l1num]);
       //}
       dd_enqueTime[chunk->sequence.l1num] = ((tsf_dp.tv_sec*1.0e9)+tsf_dp.tv_nsec);
       dd_qProcessTime[chunk->sequence.l1num] = dd_enqueTime[chunk->sequence.l1num]-dd_dequeTime[chunk->sequence.l1num];
       dd_ctr++;
    }
  ...
  clock_gettime(cidd,&End);
  End_sec = End.tv_sec;
  End_ns  = End.tv_nsec;

  threadtime = (End_sec - St_sec);
 
  pthread_mutex_lock(&fllock);
   printf("%d) Deduplicate thread end time %lu sec\n",dtid,threadtime);
  pthread_mutex_unlock(&fllock);
...
void *FragmentRefine(void * targs) {
  ...
  atomic_uint fr_dq_ctr = 0;
  atomic_uint fr_enq_ctr = 0; 
  ...
  struct timespec tsf_deq,tsf_p;

  atomic_uint frtid = syscall(SYS_gettid); 


  clock_gettime(CLOCK_REALTIME,&startT);//measure thread elapsed time
  ...
  while (TRUE) {
    ...
    pthread_mutex_lock(&fllock);
    clock_gettime(cidf,&tsf_deq);
    //if (chunk->sequence.l1num == 5){
    //   printf("%d) FragmentRefine chunk 5 dequeue time:%lu,%lu,total=%.2f ns\n",frtid,tsf_deq.tv_sec,tsf_deq.tv_nsec,((tsf_deq.tv_sec*1.0e9)+tsf_deq.tv_nsec));
    //   printf("%d) FragmentRefine chunk 5 Wait Time=%.2f nanoseconds\n",frtid,((tsf_deq.tv_sec*1.0e9)+tsf_deq.tv_nsec )-(f_enqueTime[chunk->sequence.l1num]));
    //}
    fr_dequeTime[chunk->sequence.l1num] = ((tsf_deq.tv_sec*1.0e9)+tsf_deq.tv_nsec);
    fr_qWaitTime[chunk->sequence.l1num] = (fr_dequeTime[chunk->sequence.l1num] - f_enqueTime[chunk->sequence.l1num]);  
    pthread_mutex_unlock(&fllock); 
    ...
    do {
      ...
      if(offset < chunk->uncompressed_data.n) {
        ...
        clock_gettime(cidf,&tsf_p);
      //  if (chunk->sequence.l1num == 5){
      //    printf("%d) FragmentRefine chunk 5 enqueue time:%lu,%lu,total=%.2f ns\n",frtid,tsf_p.tv_sec,tsf_p.tv_nsec,((tsf_p.tv_sec*1.e09)+tsf_p.tv_nsec));
      //    printf("%d) FragmentRefine chunk 5 Process time=%.2f nanoseconds\n",frtid,((tsf_p.tv_sec*1.e09)+tsf_p.tv_nsec) - fr_dequeTime[chunk->sequence.l1num] );
      //  }
        fr_enqueTime[chunk->sequence.l1num] = ((tsf_p.tv_sec*1.e09)+tsf_p.tv_nsec);
        fr_qProcessTime[chunk->sequence.l1num] = fr_enqueTime[chunk->sequence.l1num]-fr_dequeTime[chunk->sequence.l1num];
        fr_ctr++;
  ...
  clock_gettime(CLOCK_REALTIME,&endT);//measure thread elapsed time

  pthread_mutex_lock(&fllock);
   printf("%d) FragmentRefine thread end time %lu sec\n",frtid,(endT.tv_sec - startT.tv_sec));
  pthread_mutex_unlock(&fllock);
...
void *Fragment(void * targs){
  ...
  atomic_int rinctr =0; 
  atomic_int printonce=1; 
  struct timespec startT,endT;
  u_long elapsedT;  
  u_long sum = 0;
  u_long avg = 0;
  atomic_uint f_enqueItems = 0;
  pthread_t tid;
      ...
      if(ANCHOR_JUMP < chunk->uncompressed_data.n) {
        ...
        if(offset == 0) {
          ...
          if(ringbuffer_isFull(&send_buf)) {
            ...
            pthread_mutex_lock(&fllock);
            clock_gettime(cidf,&tsf_enq);
            //if (chunk->sequence.l1num == 5){
            //   printf("%d) Fragment chunk 5 enqueue time:%lu,%lu,total=%.2f ns\n",ftid,tsf_enq.tv_sec,tsf_enq.tv_nsec,((tsf_enq.tv_sec*1.0e9)+tsf_enq.tv_nsec));
            //   printf("%d) Fragment chunk 5 refine QUEUE : ",ftid); 
            //}
            f_enqueTime[chunk->sequence.l1num] = ((tsf_enq.tv_sec*1.0e9)+tsf_enq.tv_nsec); 
            f_enqueItems++;
            f_ctr++;
            pthread_mutex_unlock(&fllock);
          }
...
//drain buffer
  ...
  clock_gettime(CLOCK_REALTIME,&endT);//measure thread elapsed time
  printf("%d) Fragment thread end time %lu sec\n",ftid,(endT.tv_sec - startT.tv_sec));
  return NULL;
}
#endif //ENABLE_PTHREADS
...
void *Reorder(void * targs) {
  ...
  atomic_uint w_dq_ctr = 0;
  atomic_uint elapsedT;  

  struct timespec te,startT,endT;

  clock_gettime(CLOCK_REALTIME,&startT);//measure thread elapsed time
  atomic_uint wtid = syscall(SYS_gettid); 
  ...
  while(1) {
    ...
    if(chunk->isLastL2Chunk){
        clock_gettime(cidf,&tsf_rdeq);
        //if (chunk->sequence.l1num == 5){
        // printf("%d) Reorder chunk 5 dequeue time:%lu,%lu,total=%.2f ns\n",wtid,tsf_rdeq.tv_sec,tsf_rdeq.tv_nsec,((tsf_rdeq.tv_sec*1.0e9)+tsf_rdeq.tv_nsec));
        // printf("%d) Reorder chunk 5 Wait Time= %.2f nanoseconds\n",wtid,((tsf_rdeq.tv_sec*1.0e9)+tsf_rdeq.tv_nsec)- c_enqueTime[chunk->sequence.l1num]);
        //}
        r_dequeTime[chunk->sequence.l1num] = ((tsf_rdeq.tv_sec*1.0e9)+tsf_rdeq.tv_nsec);
        r_qWaitTime[chunk->sequence.l1num] = r_dequeTime[chunk->sequence.l1num] - c_enqueTime[chunk->sequence.l1num];
        r_ctr++;
    }
    ...
    do {
      ...
      //if(chunk->isLastL2Chunk){
      //  clock_gettime(cidf,&tsf_rp);
      //  r_writeTime[chunk->sequence.l1num] = ((tsf_rp.tv_sec*1.0e9)+tsf_rp.tv_nsec);
      //  r_qProcessTime[chunk->sequence.l1num] = (r_writeTime[chunk->sequence.l1num]-r_dequeTime[chunk->sequence.l1num]);
      //}
      if(chunk->header.isDuplicate) {
        free(chunk);
        chunk=NULL;
  ...
  clock_gettime(CLOCK_REALTIME,&endT);//measure thread elapsed time
  printf("%d) Reorder thread end time %lu sec\n",wtid,(endT.tv_sec - startT.tv_sec));
  printf("\n");
  return NULL;
}
#endif //ENABLE_PTHREADS
...
void *MonitorQ(void * targs){
  ...
  atomic_ulong lf_ctr=0;
  atomic_ulong lfr_ctr=0;
  atomic_ulong ldd_ctr=0;
  atomic_ulong lc_ctr=0;
  atomic_ulong lr_ctr=0;
  
  long double sumfragr;
  long double averagefr;
  long double sumddp; 
  long double avgddp; 
  long double sumcmp; 
  long double avgcmp; 
  long double sumreorder; 
  long double avgreorder; 

 
  atomic_uint wtid = syscall(SYS_gettid); 
  printf("%d) Monitor queues tid\n",wtid);
  //sleep(0.5); 
  while ( !queue_isAlive(&deduplicate_que[qid])){
    
          rv = queue_check(&deduplicate_que[qid]); 
          dd_qsize[ctr] = rv;

          rv=0;
          rv = queue_check(&refine_que[qid]);
          fr_qsize[ctr] = rv;
          
          rv=0;
          rv = queue_check(&compress_que[qid]);
          c_qsize[ctr] = rv;

          rv=0;
          rv = queue_check(&reorder_que[qid]);
          r_qsize[ctr] = rv;
        ...
        pthread_mutex_lock(&fllock);
          ...
          if (lfr_ctr != 0){
           for (i=0; i<lfr_ctr;i++){
            sumfragr = sumfragr + fr_qWaitTime[i];
           }
          averagefr = sumfragr/(lfr_ctr);
          fr_qWaitTimeInterval[ctr] = averagefr; 
          printf("\nFragment Refine Average Queue Wait time      = %.2Lf nanoseconds\n",averagefr); 
          }
          ...
          if (lfr_ctr != 0) {
            for (i=0; i<lfr_ctr;i++){
             sumfragr = sumfragr + fr_qProcessTime[i];
            }
          averagefr = sumfragr/(lfr_ctr);
          fr_qProcessTimeInterval[ctr] = averagefr; 
          printf("Fragment Refine Average Process time         = %.2Lf nanoseconds\n",averagefr); 
          }

          sumddp=0;
          if(ldd_ctr != 0){ 
          for (i=0; i<ldd_ctr;i++){
           sumddp = sumddp + dd_qWaitTime[i];
          }
          avgddp = sumddp/(ldd_ctr);
          dd_qWaitTimeInterval[ctr] = avgddp; 
          printf("Dedup Average Wait time                      = %.2Lf nanoseconds\n",avgddp); 
          }
          ...
          if(ldd_ctr != 0){ 
          for (i=0; i<ldd_ctr;i++){
            sumddp = sumddp + dd_qProcessTime[i];
          }
          avgddp = sumddp/(ldd_ctr);
          dd_qProcessTimeInterval[ctr]=avgddp; 
          printf("Dedup Average Process time                   = %.2Lf nanoseconds\n",avgddp); 
          }

          sumcmp=0;
          if(lc_ctr != 0) { 
          for (i=0; i<lc_ctr;i++){
            sumcmp = sumcmp + c_qWaitTime[i];
          }
          avgcmp = sumcmp/(lc_ctr);
          c_qWaitTimeInterval[ctr] = avgcmp; 
          printf("Compress Average Wait time                   = %.2Lf nanoseconds\n",avgcmp); 

          sumcmp=0;
          avgcmp=0; 
          for (i=0; i<lc_ctr;i++){
            sumcmp = sumcmp + c_qProcessTime[i];
          }
          avgcmp = sumcmp/(lc_ctr);
          c_qProcessTimeInterval[ctr] = avgcmp; 
          printf("Compress Average Process time                = %.2Lf nanoseconds\n",avgcmp); 
          }

         sumreorder=0;
         if(lr_ctr !=0) { 
         for (i=0; i<lr_ctr;i++){
           sumreorder = sumreorder + r_qWaitTime[i];
         }
         avgreorder = sumreorder/(lr_ctr);
         r_qWaitTimeInterval[ctr] = avgreorder; 
         printf("Reorder Average Wait time                    = %.2Lf nanoseconds\n",avgreorder); 
         }
         ...
         //if(lr_ctr !=0) { 
         //for (i=0; i<lr_ctr;i++){
         //  sumreorder = sumreorder + r_qProcessTime[i];
         //}
         //avgreorder = sumreorder/(lr_ctr);
         //r_qProcessTimeInterval[ctr] = avgreorder; 
         //printf("Reorder Average Process time                 = %.2Lf nanoseconds\n",avgreorder); 
         //}
         //printf("Counters F:%lu FR:%lu DD:%lu C:%lu R:%lu\n",f_ctr,fr_ctr,dd_ctr,c_ctr,r_ctr); 
        pthread_mutex_unlock(&fllock);
...
void Encode(config_t * _conf) {
  ...
  printf("deduplicate_que sizeof=%lu bytes\n",(sizeof(queue_t)*nqueues));
  ...
  pthread_t threads_anchor[MAX_THREADS],
    ...
    threads_send, threads_process, threads_monitor;
  ...
  pthread_create(&threads_monitor, NULL, MonitorQ, &data_process_args);
  ...
  //join all threads 
  pthread_join(threads_monitor, NULL);
  ...
  long double sumfr=0;
  printf("fr_ctr=%lu\n",fr_ctr);  
  for (i=0; i<fr_ctr-2;i++){
      sumfr = sumfr + fr_qWaitTime[i];
  }
  long double avgfr = sumfr/(fr_ctr-1); 
  printf("\nFragment Refine Average total Wait time    = %.2Lf microseconds\n",(avgfr*1.0e-3)); 
  

  sumfr=0;
  avgfr=0;
  for (i=0; i<fr_ctr-1;i++){
      sumfr = sumfr + fr_qProcessTime[i];
  }
  avgfr = sumfr/(fr_ctr-1); 
  printf("Fragment Refine Average total Process time = %.2Lf microseconds\n",(avgfr*1.0e-3)); 


  long double sumdd=0; 
  for (i=0; i<dd_ctr-1;i++){
      sumdd = sumdd + dd_qWaitTime[i];
  }
  long double avgdd = sumdd/(dd_ctr-1); 
  printf("Dedup Average total Wait time              = %.2Lf microseconds\n",(avgdd*1.0e-3)); 
  

  sumdd=0;
  avgdd=0; 
  for (i=0; i<dd_ctr-1;i++){
      sumdd = sumdd + dd_qProcessTime[i];
  }
  avgdd = sumdd/(dd_ctr-1); 
  printf("Dedup Average total Process time           = %.2Lf microseconds\n",(avgdd*1.0e-3)); 

  long double sumc=0; 
  for (i=0; i<c_ctr-1;i++){
      sumc = sumc + c_qWaitTime[i];
  }
  long double avgc = sumc/(c_ctr-1); 
  printf("Compress Average total Wait time           = %.2Lf microseconds\n",(avgc*1.0e-3)); 

  sumc=0;
  avgc=0; 
  for (i=0; i<c_ctr-1;i++){
      sumc = sumc + c_qProcessTime[i];
  }
  avgc = sumc/(c_ctr-1); 
  printf("Compress Average total Process time        = %.2Lf microseconds\n",(avgc*1.0e-3)); 


  long double sumr=0; 
  for (i=0; i<r_ctr-1;i++){
      sumr = sumr + r_qWaitTime[i];
  }
  long double avgr = sumr/(r_ctr-1); 
  printf("Reorder Average total Wait time            = %.2Lf microseconds\n",(avgr*1.0e-3)); 
 
  //sumr=0;
  //avgr=0; 
  //for (i=0; i<r_ctr-1;i++){
  //    sumr = sumr + r_qProcessTime[i];
  //}
  //avgr = sumr/(r_ctr-1); 
  //printf("Reorder Average total Write to file time   = %.2Lf microseconds\n",(avgr*1.0e-3)); 
...
#endif
// Write Queue stats out to file

  FILE *fp;
#define MAX_SAMPLING 15

  fp = fopen("QueueWaitTimes.dat","w+");
  fprintf(fp,"#yaxis=QueuesWaitTimes-microseconds\n");
  fprintf(fp,"#xaxis=Time-seconds\n");
  fprintf(fp,"#xlabels=\n");
  fprintf(fp,"#showlabels=1\n");
  fprintf(fp,"#exactx=1\n");
  fprintf(fp,"#format=eps,png\n");
  fprintf(fp,"x,FR_QueueWait,Dedup_QueueWait,Compress_QueueWait,Reorder_QueueWait,FR_QueueWait-err,Dedup_QueueWait-err,Compress_QueueWait-err,Reorder_QueueWait-err\n");
  for (i=0; i < monitor_ctr ;i++){
    if (fr_qWaitTimeInterval[i] !=0){
      fprintf(fp,"%d,%f,%f,%f,%f,0,0,0,0\n",i,(fr_qWaitTimeInterval[i]*1.0e-3),(dd_qWaitTimeInterval[i]*1.0e-3),(c_qWaitTimeInterval[i]*1.0e-3),(r_qWaitTimeInterval[i]*1.0e-3));
    }
  }
  
  fclose(fp);

  fp = fopen("ProcessTimes.dat","w+");
  fprintf(fp,"#yaxis=ProcessTimes-microseconds\n");
  fprintf(fp,"#xaxis=Time-seconds\n");
  fprintf(fp,"#xlabels=\n");
  fprintf(fp,"#showlabels=1\n");
  fprintf(fp,"#exactx=1\n");
  fprintf(fp,"#format=eps,png\n");
  fprintf(fp,"x,FR_ProcessT,Dedup_ProcessT,Compress_ProcessT,Reorder_ProcessT,FR_ProcessT-err,Dedup_ProcessT-err,Compress_ProcessT-err,Reorder_ProcessT-err\n");
  for (i=0; i < monitor_ctr ;i++){
   if (fr_qProcessTimeInterval[i] != 0){
    fprintf(fp,"%d,%f,%f,%f,%f,0,0,0,0\n",i,(fr_qProcessTimeInterval[i]*1.0e-3),(dd_qProcessTimeInterval[i]*1.0e-3),(c_qProcessTimeInterval[i]*1.0e-3),(r_qProcessTimeInterval[i]*1.0e-3));
   }
  }
  fclose(fp);

  
#ifdef ENABLE_PRINT_DEBUG_C
  printf("\ndd enque times\n"); 
  for (i=0; i<dd_ctr;i++){
      printf("%d)%lu\n",i,dd_enqueTime[i]);
  }

  printf("\nCompress deque times\n"); 
  for (i=0; i<c_ctr;i++){
      printf("%d)%lu\n",i,c_dequeTime[i]);
  }
  ...
  fp = fopen("QueueSize.dat","w+");
  fprintf(fp,"#yaxis=QueueSize\n");
  fprintf(fp,"#xaxis=Time-seconds\n");
  fprintf(fp,"#xlabels=\n");
  fprintf(fp,"#showlabels=1\n");
  fprintf(fp,"#exactx=1\n");
  fprintf(fp,"#format=eps,png\n");
  fprintf(fp,"x,Refine_Q,Dedup_Q,Compress_Q,Reorder_Q,Refine_Q-err,Dedup_Q-err,Compress_Q-err,Reorder_Q-err\n");
  for (i=0; i < monitor_ctr; i++){
      fprintf(fp,"%d,%lu,%lu,%lu,%lu,0,0,0,0\n",i,(QUEUE_SIZE-fr_qsize[i]),(QUEUE_SIZE-dd_qsize[i]),(QUEUE_SIZE-c_qsize[i]),(QUEUE_SIZE-r_qsize[i]));
  }
  fclose(fp);

  //ProfilerStop();
  //endT = clock_gettime(CLOCK_REALTIME);
}
