...
with open ('pid.dat','w') as ofile:
 ...
 ofile.write("%s\n" % ("#exactx=1"))
 ofile.write("%s\n" % ("#format=eps,png"))
 ofile.write("%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" % ("x","FragmentpCPU","FRpCPU","FRpCPU-2","DeduppCPU","DeduppCPU-2","CompresspCPU","CompresspCPU-2","ReorderpCPU","FragmentpCPU-err","FRpCPU-err","FRpCPU-2-err","DeduppCPU-err","DeduppCPU-2-err","CompresspCPU-err","CompresspCPU-2-err","ReorderpCPU-err"))
 with open ('pcpu2.dat','r') as fh:
    for line in fh:
      ofile.write("%s,0,0,0,0,0,0,0\n" % line.strip('\n'))  #need one less 0 for lchart given that one was inserted above
