...
with open ('pid.dat','w') as ofile:
 ...
 ofile.write("%s\n" % ("#exactx=1"))
 ofile.write("%s\n" % ("#format=eps,png"))
 ofile.write("%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" % ("x","FragmentpCPU","FRpCPU","DeduppCPU","CompresspCPU","ReorderpCPU","MonitorpCPU","FragmentpCPU-err","FRpCPU-err","DeduppCPU-err","CompresspCPU-err","ReorderpCPU-err","MonitorpCPU-err"))
 with open ('pcpu2.dat','r') as fh:
    for line in fh:
      ofile.write("%s,0,0,0,0,0\n" % line.strip('\n'))  #need one less 0 for lchart given that one was inserted above
